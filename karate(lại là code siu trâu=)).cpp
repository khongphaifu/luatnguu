...
void chau(){
    ...
    for(ll i=0;i<n;i++){
        cin>>x>>b[i];
        if(b[i]==0) nam.push_back(x);
        else nu.push_back(x);
    }
    stack<ll> st1;stack<ll> st2;
    vector<ll> ans;
    ans.push_back(0);
    ll temp=0;
    for(ll i=1;i<nam.size();i++){
        if(nam[i]>nam[i-1]) {
            while(!st1.empty() && nam[i]>st1.top()){
                temp+=st2.top();
                st1.pop();st2.pop();
            }
            temp++;
            ...
            st1.push(nam[i-1]);st2.push(temp+1);temp=0;
        }
        ans.push_back(temp);
    }
    vector<ll> ans1;
    ...
    ans1.push_back(0);
    temp=0;
    for(ll i=1;i<nu.size();i++){
        if(nu[i]>nu[i-1]) {
            while(!st11.empty() && nu[i]>st11.top()){
                temp+=st22.top();
                st11.pop();st22.pop();
            }
            temp++;
            ...
            st11.push(nu[i-1]);st22.push(temp+1);temp=0;
        }
        ans1.push_back(temp);
    }
