// Program link: https://tinhoclk.com/problem/coffee
// Program name: v 
#define TASK  "COFFEE"
// sol: 
...
map<pair<ll,ll>,ll> mp,mpc1,mpc2;
...
{   
    ...
    void sol()
    {
        ...
        for(auto it:mp)
        {
            ll a=it.fi.fi,b=it.fi.se;
            if(a==par[b])
            {
                ans+=min(f[b]*mpc1[{a,b}],mpc2[{a,b}]);
            }
            else
            {
                ans+=min(f[a]*mpc1[{a,b}],mpc2[{a,b}]);
            }
        }
        cout<<ans;
    }       
